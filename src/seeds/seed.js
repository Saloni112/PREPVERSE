...
// Import models using ES6 imports
import interviewQue from '../models/interviewQue.js';
import Topic from '../models/Topic.js';
import Question from '../models/Question.js';

// Your completeQuestionsData array (all your interview questions)
...
const aptitudeTopicsData = [
  {
    name: "Number System",
    slug: "number-system",
    description: "Questions related to number theory and properties"
  },
  {
    name: "Percentages", 
    slug: "percentages",
    description: "Percentage calculation problems"
  },
  {
    name: "Number Series",
    slug: "number-series", 
    description: "Number sequence and pattern problems"
  },
  {
    name: "Seating Arrangement",
    slug: "seating-arrangement",
    description: "Logical seating arrangement problems"
  },
  {
    name: "Reading Comprehension",
    slug: "reading-comprehension",
    description: "Reading and comprehension exercises"
  },
  {
    name: "Para Jumbles",
    slug: "para-jumbles",
    description: "Paragraph rearrangement problems"
  }
];
...
const aptitudeQuestionsData = [
  ...
  },
  {
    ...
    options: ["0", "1", "2", "3"],
    answer: "1", 
    explanation: "1 is neither prime nor composite as it has only one positive divisor.",
    difficulty: "medium"
  },
  {
    topicId: null,
  ...
  {
    topicId: null,
    question: "A number increased by 25% gives 100. What is the number?",
    options: ["75", "80", "85", "90"],
    answer: "80",
    explanation: "Let number be x. Then 1.25x = 100, so x = 100/1.25 = 80",
    difficulty: "medium"
  },
  {
    topicId: null,
    question: "If a shirt costs $40 after 20% discount, what was original price?",
    options: ["$48", "$50", "$52", "$55"],
    answer: "$50",
    explanation: "Let original price be x. Then 0.8x = 40, so x = 40/0.8 = 50",
    difficulty: "medium"
  },
  ...
  // Number Series Questions
  {
    ...
    options: ["9", "10", "11", "12"],
    answer: "10",
    explanation: "Even number sequence: +2 each time",
    difficulty: "easy"
  },
  {
    topicId: null,
    ...
    options: ["20", "24", "25", "30"],
    answer: "25",
    explanation: "Square number sequence: 1², 2², 3², 4², 5² = 25",
    difficulty: "medium"
  },
  {
    topicId: null,
    ...
    options: ["13", "15", "17", "19"],
    answer: "13",
    explanation: "Prime number sequence: 2, 3, 5, 7, 11, 13",
    difficulty: "medium"
  },
  ...
  {
    topicId: null,
    question: "In a row of 5 people, A is to the left of B but to the right of C. D is to the right of B but to the left of E. Who is in the middle?",
    options: ["A", "B", "C", "D"],
    answer: "B",
    explanation: "Arrangement: C, A, B, D, E. So B is in the middle.",
    difficulty: "medium"
  },
  ...
  {
    topicId: null,
    question: "Read the passage: 'The quick brown fox jumps over the lazy dog. This sentence contains all letters of the alphabet.' What is special about this sentence?",
    options: [
    ...
    answer: "It contains all alphabet letters",
    explanation: "The sentence 'The quick brown fox jumps over the lazy dog' is known as a pangram - it contains all 26 letters of the English alphabet.",
    difficulty: "easy"
  },
  ...
  {
    topicId: null,
    question: "Arrange the sentences: 1. Then he went to school. 2. First, he brushed his teeth. 3. After that, he had breakfast. 4. Finally, he reached school.",
    options: [
      "2-3-1-4",
      "1-2-3-4", 
      "3-2-1-4",
    ...
    answer: "2-3-1-4",
    explanation: "Correct sequence: Brush teeth (2) → Breakfast (3) → Go to school (1) → Reach school (4)",
    difficulty: "easy"
  }
];
...
async function seedInterviewQuestions() {
  try {
    // Connect to MongoDB
    const mongoURI = process.env.MONGODB_URI || 'mongodb://localhost:27017/WorkWaveDB';
    ...
    // Clear data in sequence to avoid foreign key issues
    await Question.deleteMany({});
    await Topic.deleteMany({});
    await interviewQue.deleteMany({});
    
    console.log('✅ Cleared existing data');

    // Seed Interview Questions
    console.log('📝 Seeding interview questions...');
    const interviewResult = await interviewQue.insertMany(completeQuestionsData);
    console.log(`✅ Added ${interviewResult.length} interview questions`);

    // Seed Aptitude Topics
    console.log('📚 Seeding aptitude topics...');
    const topicsResult = await Topic.insertMany(aptitudeTopicsData);
    console.log(`✅ Added ${topicsResult.length} aptitude topics`);

    // Map topic slugs to IDs for question assignment
    const topicMap = {};
    topicsResult.forEach(topic => {
      topicMap[topic.slug] = topic._id;
    });

    // Assign correct topicIds to aptitude questions
    const questionsWithTopicIds = aptitudeQuestionsData.map((q, index) => {
      let topicSlug;
      if (index < 4) topicSlug = "number-system";
      else if (index < 8) topicSlug = "percentages";
      else if (index < 11) topicSlug = "number-series";
      else if (index < 12) topicSlug = "seating-arrangement";
      else if (index < 13) topicSlug = "reading-comprehension";
      else topicSlug = "para-jumbles";
    ...
    const questionsResult = await Question.insertMany(questionsWithTopicIds);
    console.log(`✅ Added ${questionsResult.length} aptitude questions`);

    // Show Interview Questions Summary
    const roleSummary = await interviewQue.aggregate([
      { $group: { _id: '$role', count: { $sum: 1 } } },
      { $sort: { count: -1 } }
    ]);
    
    console.log('\n📈 Interview Questions Summary by role:');
    roleSummary.forEach(role => {
      console.log(`   ${role._id}: ${role.count} questions`);
    });

    const interviewTopicSummary = await interviewQue.aggregate([
      { $group: { _id: '$topic', count: { $sum: 1 } } },
      { $sort: { count: -1 } }
    ]);
    
    console.log('\n📊 Interview Questions Summary by topic:');
    interviewTopicSummary.forEach(topic => {
      console.log(`   ${topic._id}: ${topic.count} questions`);
    });

    // Show Aptitude Summary
    const aptitudeTopicSummary = await Topic.aggregate([
      { 
        $lookup: {
          from: 'questions',
          localField: '_id',
    ...
    console.log('\n🎯 Aptitude Module Summary:');
    aptitudeTopicSummary.forEach(topic => {
      console.log(`   ${topic.name}: ${topic.questionCount} questions`);
    });

    const difficultySummary = await Question.aggregate([
      { $group: { _id: '$difficulty', count: { $sum: 1 } } },
      { $sort: { _id: 1 } }
    ]);
    
    console.log('\n📊 Aptitude Questions by difficulty:');
    difficultySummary.forEach(diff => {
    ...
    console.log('====================================');
    console.log(`📝 Total Interview Questions: ${interviewResult.length}`);
    console.log(`📚 Total Aptitude Topics: ${topicsResult.length}`);
    console.log(`🧮 Total Aptitude Questions: ${questionsResult.length}`);
    console.log('====================================');
    console.log('🚀 You can now run your application and all data will be loaded from the database.');
